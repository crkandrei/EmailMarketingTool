        ...
        if(request()->ajax()){
            return datatables()->of(Group::where('user_id', Auth::user()->id)->get())
                ...
                ->addColumn('action', function($data){
                    $button = '<button type="button" name="add_customers_to_group" id="'.$data->id.'" class="customers-to-group text-black btn btn-success btn-sm">Add users to group</button>';
                    $button .= '&nbsp;&nbsp;';
            ...
            return view('data.group')->with('customers',Customer::where('user_id', Auth::user()->id)->get());
        }
    }
